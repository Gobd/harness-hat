...
To create a fresh session for an explicit directory without attaching, use `hat sh new --path DIRECTORY`.
...
```sh
hat ws open codium          # use the current directory's session
hat sh <ID> open codium     # use a session by integer ID
```
...
3. Select the running Harness Hat container for your project. Use `hat sh` in a **terminal** to list the Harness Hat session ID and Docker container ID when more than one is running.
...
- **No Harness Hat container appears:** run `hat sh` in a **terminal** and confirm the session is listed. Start one with `hat ws` if needed.
