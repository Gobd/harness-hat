...
# Bump by updating ARGs and sha256s from:
#   https://dotnetcli.blob.core.windows.net/dotnet/release-metadata/10.0/releases.json
ARG DOTNET10_VERSION=10.0.302
ARG DOTNET10_SHA256_X64=10069bec8783596484a610332f090d562802a41b9b40e3327a5a5688b572e10c296ae300f940d40461f23c157ed1b0843c2f8e6b3f20d8d8d9d83432d8143bac
ARG DOTNET10_SHA256_AARCH64=9e409c14e00686d661c78fa4dd9ad0e4dcf695c328bd5ff777d05b4a9c34b42cf89b12573b92e9fb2f565dbe12016b4835f77c7d9a42b55a7494df21634cd5d6

RUN set -eu; \
    case "${TARGETARCH:-$(dpkg --print-architecture)}" in \
        amd64|x86_64)  dotnet_arch="x64";   dotnet_sha="${DOTNET10_SHA256_X64}" ;; \
        arm64|aarch64) dotnet_arch="arm64"; dotnet_sha="${DOTNET10_SHA256_AARCH64}" ;; \
    ...
    echo "${dotnet_sha}  /tmp/dotnet.tar.gz" | sha256sum -c -; \
    mkdir -p "${DOTNET_ROOT}"; \
...
ARG DOTNET8_SHA256_X64=e94513dfe42271a85f01e87bd4272aa80b4ec13556f4531754802542225667775242c5e281a94837dae6cc65f7bcc457d2f663f240c0e2b7573fd909e786b1a5
ARG DOTNET8_SHA256_AARCH64=8c6dd335a8fa63849af551fe6f10ca8e92db0b1aaa761727e3d997d7ebfebe68d9fdccdd241a1804f6812055770b00a40648f32e69f1606ecf864440902c67a1

RUN set -eu; \
    case "${TARGETARCH:-$(dpkg --print-architecture)}" in \
        amd64|x86_64)  dotnet_arch="x64";   dotnet_sha="${DOTNET8_SHA256_X64}" ;; \
        arm64|aarch64) dotnet_arch="arm64"; dotnet_sha="${DOTNET8_SHA256_AARCH64}" ;; \
        *) echo "unsupported architecture: ${TARGETARCH:-$(dpkg --print-architecture)}" >&2; exit 1 ;; \
    esac; \
    curl -fsSL \
        -o /tmp/dotnet.tar.gz \
