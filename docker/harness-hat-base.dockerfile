...
#
# SECURITY (H5): trust here rests on TLS to claude.ai — the upstream installer
# has no published checksum to pin against, and pinning a version would defeat
# the "always newest model" intent above. The GPG-verified `claude-stable` apt
# build (installed as root above) is the verified fallback if this build is ever
# in doubt. We download to a file before executing so a truncated/interrupted
# download cannot run as a partial script (pipe-to-bash has no such guarantee).
RUN set -eu; \
